					...
					for (int j = 1; j <= width; j++) {
						...
						for (SegNode sn : segGraph) {
							if (i == sn.getRow() && j == sn.getCol()) {
								String prompt = "pos=" + POSTag.int2str(sn.getPos()) + " weight=" + sn.getWeight() + "\n";
								html.append("<td width=\"" + (95 / (double) width) + "%\"><a title=\"" + prompt + "\">" + sn.getWord() + "</a></td>");
								flag = true;
								break;
							}
						}
				...
				for (int i = 0; i <= height; i++) {
					...
					for (int j = 1; j <= width; j++) {
						...
						for (SegNode sn : optSegGraph) {
							if (i == sn.getRow() && j == sn.getCol()) {
								html.append("<td width=\"" + (95 / (double) width) + "%\"><a title=\"pos=" + POSTag.int2str(sn.getPos()) + " weight="
										+ sn.getWeight() + "\">" + sn.getWord() + "</a></td>");
								flag = true;
								break;
							}
						}
